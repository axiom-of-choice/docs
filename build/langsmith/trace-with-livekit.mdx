...
Use the same `thread_id` for both calls and for `set_thread_id`, so the integration can match the recording to the conversation. Because `set_thread_id` stores the id in a `ContextVar`, a worker handling many sessions concurrently keeps them correctly separated, since each runs in its own `@server.rtc_session()` task.
