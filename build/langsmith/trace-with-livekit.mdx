...
@server.rtc_session()
async def my_agent(ctx: agents.JobContext):
    set_thread_id(ctx.room.name)  # a stable id for this conversation
    ...
```

The integration then stamps the id on every span for you. Because `set_thread_id` stores the id in a `ContextVar`, a worker handling many sessions concurrently keeps them correctly separated, since each runs in its own `@server.rtc_session()` task.
