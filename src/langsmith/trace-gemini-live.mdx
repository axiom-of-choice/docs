...
<Note>
The ADK Live integration requires `langsmith[google-adk-live]` (separate from the `langsmith[google-adk]` batch integration, so non-streaming ADK users do not take on its dependencies). To trace ADK's non-streaming paths (text agents, tools, multi-agent workflows), see [Trace Google ADK applications](/langsmith/trace-with-google-adk).
</Note>
