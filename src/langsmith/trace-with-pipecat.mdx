...
<Note>
If your LLM stage is an in-process [LangGraph or LangChain](/oss/langgraph/overview) agent, pass `configure_pipecat(llm_span_kind="chain")` and set `LANGSMITH_TRACING_MODE=otel` in the environment. The span kind tells the integration that Pipecat's `llm` span only orchestrates the graph, and the OTel mode routes the graph's model and tool runs through the same OpenTelemetry provider so they nest inside that span. Without the OTel mode, those runs post to LangSmith directly and form a separate trace instead of nesting.
</Note>
