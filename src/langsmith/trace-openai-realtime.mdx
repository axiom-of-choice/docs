...
Which integration you use follows from how you built your agent, not a preference: if you drive the raw WebSocket connection yourself, use `wrap_realtime`; if you build with the OpenAI Agents SDK, use `wrap_realtime_session`. Each is covered in its own section below.
...
<Note>
Each conversation is captured as its own trace. To group it with related interactions in a LangSmith [thread](/langsmith/threads), for example to continue an earlier session or pick up from a text chat, pass a `thread_id`. Reusing the same id across traces links their events together. This is optional.
</Note>
